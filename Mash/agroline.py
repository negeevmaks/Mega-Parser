...
def get_category():
    ...
    for ad in ads:
        if 'https://agriline.ua/' in ad.get('href'):
            category.append(ad.get('href').replace('https://agriline.ua/-/', ''))
...
def pages(link):
    ...
    resuk.append(f'https://agriline.ua/-/{link}')
...
def start_parsing(category, pric, cares, result_folder):
    ...
    workbook_path = f'{result_folder}/agriline.xlsx'
            ...
            if pric[0] < pride and pride < pric[1]:
                if soup.find('span', {'class': 'loc-country'}).text == 'Україна':
                    if check_model_exists(link, ws) != True:
                        ...
                        wb.save(f'{result_folder}/agriline.xlsx')
