...
base_url = 'https://machineryline.ua/'
...
def get_category():
    ...
    for ad in ads:
        if 'https://machineryline.ua' in ad.get('href'):
            category.append(ad.get('href').replace('https://machineryline.ua/-/', ''))
...
def pages(link):
    ...
    resuk.append(f'https://machineryline.ua/-/{link}')
...
def check_model_exists(url, ws):
    for row in ws.iter_rows(min_row=2):  # Починаємо з другого рядка (першій рядок містить заголовки)
        cell_value = row[0].value  # Перевіряємо значення в першому стовпці (A стовпець для URL)

        if url in cell_value:
            return True  # Запис з вказаним URL знайдено

    return False  # Запис з вказаним URL не знайден
...
def start_parsing(category, pric, cares, result_folder):
    ...
    workbook_path = f'{result_folder}/machineryline.xlsx'
    ...
    try:
        ...
        os.remove(workbook_path)  # Delete the corrupted file
        create_workbook(workbook_path)  # Create a new workbook
        wb = openpyxl.load_workbook(workbook_path)
        ...
        if price != 'за запитом':
            ...
            if pric[0] < pride and pride < pric[1]:
                if soup.find('span', {'class': 'loc-country'}).text == 'Україна':
                    if check_model_exists(link, ws) != True:
                        ...
                        wb.save(f'{result_folder}/machineryline.xlsx')
